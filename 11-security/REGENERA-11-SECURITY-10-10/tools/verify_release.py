...
required_release={"MANIFEST.json","SBOM.json","BUILD-PROVENANCE.json","CONTROL-STATUS.json","PAYLOAD-CHECKSUMS.sha256"}
if not required_release.issubset({p.name for p in release.iterdir() if p.is_file()}): errors.append("release-evidence-incomplete")
if errors:
